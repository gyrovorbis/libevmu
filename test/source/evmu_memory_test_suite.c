...
GBL_RESULT EvmuMemoryTestSuite_readIntInvalid_(GblTestSuite* pSelf, GblContext* pCtx) {
    ...
    GBL_TEST_COMPARE(EvmuMemory_readData(pSelf_->pMemory, 0xffff), 0);
...
GBL_RESULT EvmuMemoryTestSuite_readProgramInvalid_(GblTestSuite* pSelf, GblContext* pCtx) {
    GBL_CTX_BEGIN(pCtx);

    EvmuMemoryTestSuite_* pSelf_ = EVMU_MEMORY_TEST_SUITE_(pSelf);
    GBL_TEST_EXPECT_ERROR();

    GBL_TEST_COMPARE(EvmuMemory_readProgram(pSelf_->pMemory, EVMU_FLASH_SIZE), 0x0);
    GBL_TEST_COMPARE(GBL_CTX_LAST_RESULT(), GBL_RESULT_ERROR_OUT_OF_RANGE);
    GBL_CTX_CLEAR_LAST_RECORD();
...
GBL_RESULT EvmuMemoryTestSuite_writeProgramInvalid_(GblTestSuite* pSelf, GblContext* pCtx) {
    GBL_CTX_BEGIN(pCtx);

    EvmuMemoryTestSuite_* pSelf_ = EVMU_MEMORY_TEST_SUITE_(pSelf);
    GBL_TEST_EXPECT_ERROR();

    GBL_TEST_COMPARE(EvmuMemory_writeData(pSelf_->pMemory, EVMU_FLASH_SIZE, 0xff),
                     GBL_RESULT_ERROR_OUT_OF_RANGE);
    GBL_CTX_CLEAR_LAST_RECORD();
    GBL_CTX_END();
}
...
GBL_RESULT EvmuMemoryTestSuite_ramBankChange_(GblTestSuite* pSelf, GblContext* pCtx) {
    ...
    const EvmuWord psw = EvmuMemory_viewData(pSelf_->pMemory, EVMU_ADDRESS_SFR_PSW);

    EvmuMemory_writeData(pSelf_->pMemory, 0x0, 0x1);
    EvmuMemory_writeData(pSelf_->pMemory, EVMU_ADDRESS_SFR_PSW, psw & ~EVMU_SFR_PSW_RAMBK0_MASK);
    GBL_TEST_COMPARE(EvmuMemory_viewData(pSelf_->pMemory, 0x0), 0);
    EvmuMemory_writeData(pSelf_->pMemory, EVMU_ADDRESS_SFR_PSW, psw | EVMU_SFR_PSW_RAMBK0_MASK);
    GBL_TEST_COMPARE(EvmuMemory_viewData(pSelf_->pMemory, 0x0), 1);
...
static EVMU_RESULT setIrbk_(EvmuMemory* pSelf, GblBool irbk0, GblBool irbk1) {
    EvmuWord psw = EvmuMemory_readData(pSelf, EVMU_ADDRESS_SFR_PSW);
    ...
    return EvmuMemory_writeData(pSelf, EVMU_ADDRESS_SFR_PSW, psw);
}
GBL_RESULT EvmuMemoryTestSuite_indirectAddressInvalid_(GblTestSuite* pSelf, GblContext* pCtx) {
    ...
    for(uint8_t addr = 0; addr < 0xf; ++addr) {
        EvmuMemory_writeData(pSelf_->pMemory, addr, addr);
...
GBL_RESULT EvmuMemoryTestSuite_wramRead_(GblTestSuite* pSelf, GblContext* pCtx) {
    ...
    EvmuMemoryTestSuite_* pSelf_ = EVMU_MEMORY_TEST_SUITE_(pSelf);
    const EvmuWord vsel = EvmuMemory_readData(pSelf_->pMemory, EVMU_ADDRESS_SFR_VSEL);
...
GBL_RESULT EvmuMemoryTestSuite_wramWrite_(GblTestSuite* pSelf, GblContext* pCtx) {
    ...
    // Test without auto-increment
    EvmuMemory_writeData(pSelf_->pMemory, EVMU_ADDRESS_SFR_VSEL, vsel&~EVMU_SFR_VSEL_INCE_MASK);
    EvmuMemory_writeData(pSelf_->pMemory, EVMU_ADDRESS_SFR_VRMAD2, 0x0);
    EvmuMemory_writeData(pSelf_->pMemory, EVMU_ADDRESS_SFR_VRMAD1, 0x0);
...
GBL_RESULT EvmuMemoryTestSuite_xramBankChangeInvalid_(GblTestSuite* pSelf, GblContext* pCtx) {
    ...
    GBL_TEST_EXPECT_ERROR();
    GBL_TEST_COMPARE(EvmuMemory_writeData(pSelf_->pMemory, EVMU_ADDRESS_SFR_XBNK, 3),
                     GBL_RESULT_ERROR_OUT_OF_RANGE);
    GBL_CTX_CLEAR_LAST_RECORD();
...
GBL_RESULT EvmuMemoryTestSuite_xramBankChange_(GblTestSuite* pSelf, GblContext* pCtx) {
    ...
    EvmuMemory_writeData(pSelf_->pMemory, EVMU_ADDRESS_SFR_XBNK, 0);
    EvmuMemory_writeData(pSelf_->pMemory, EVMU_ADDRESS_SEGMENT_XRAM_BASE, 1);
    EvmuMemory_writeData(pSelf_->pMemory, EVMU_ADDRESS_SFR_XBNK, 1);
    GBL_TEST_COMPARE(EvmuMemory_readData(pSelf_->pMemory, EVMU_ADDRESS_SEGMENT_XRAM_BASE), 0);
    EvmuMemory_writeData(pSelf_->pMemory, EVMU_ADDRESS_SEGMENT_XRAM_BASE, 2);
    EvmuMemory_writeData(pSelf_->pMemory, EVMU_ADDRESS_SFR_XBNK, 2);
    GBL_TEST_COMPARE(EvmuMemory_readData(pSelf_->pMemory, EVMU_ADDRESS_SEGMENT_XRAM_BASE), 0);
    EvmuMemory_writeData(pSelf_->pMemory, EVMU_ADDRESS_SEGMENT_XRAM_BASE, 3);
    EvmuMemory_writeData(pSelf_->pMemory, EVMU_ADDRESS_SFR_XBNK, 0);
    GBL_TEST_COMPARE(EvmuMemory_readData(pSelf_->pMemory, EVMU_ADDRESS_SEGMENT_XRAM_BASE), 1);
...
GBL_EXPORT GblType EvmuMemoryTestSuite_type(void) {
    ...
    const static GblTestCase cases[] = {
        ...
        { "readProgramInvalid",    EvmuMemoryTestSuite_readProgramInvalid_    },
        { "writeProgramInvalid",   EvmuMemoryTestSuite_writeProgramInvalid_   },
