 ...
 *  @{
 */
//! Writes a message to the log with the \c GBL_LOG_VERBOSE flag within the "evmu" domain.
#define EVMU_LOG_VERBOSE(...)   GBL_LOG_VERBOSE("evmu", __VA_ARGS__)
//! Writes a message to the log with the \c GBL_LOG_DEBUG flag within the "evmu" domain.
#define EVMU_LOG_DEBUG(...)     GBL_LOG_DEBUG("evmu", __VA_ARGS__)
//! Writes a message to the log with the \c GBL_LOG_INFO flag within the "evmu" domain.
#define EVMU_LOG_INFO(...)      GBL_LOG_INFO("evmu", __VA_ARGS__)
//! Writes a message to the log with the \c GBL_LOG_WARN flag within the "evmu" domain.
#define EVMU_LOG_WARN(...)      GBL_LOG_WARN("evmu", __VA_ARGS__)
//! Writes a message to the log with the \c GBL_LOG_ERROR flag within the "evmu" domain.
#define EVMU_LOG_ERROR(...)     GBL_LOG_ERROR("evmu", __VA_ARGS__)
//! Pushes a level to the log stack within the "evmu" domain. See \c GBL_LOG_PUSH().
#define EVMU_LOG_PUSH()         GBL_LOG_PUSH()
//! Pops n levels from the log stack within the "evmu" domain. See \c GBL_LOG_POP().
