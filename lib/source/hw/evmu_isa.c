...
EVMU_API evmuInstructionDecodeOperands(const EvmuInstruction* pInstruction, EvmuDecodedOperands* pOperands) {
    ...
    EVMU_API_VALIDATE(pInstruction);
    EVMU_API_VALIDATE(*pInstruction->bytes);
    EVMU_API_VALIDATE(pOperands);
...
#define EXTRACT_OPERAND_GENERIC(var, bitCount, srcBit, destBit, op)                     \
    do {                                                                                \
        pOperands->var op ((instrCode & GBL_BIT_MASK(bitCount, srcBit)) << destBit);    \
        instrCode >>= bitCount;                                                         \
    } while(0)
...
#define EXTRACT_OPERAND_APPEND(var, bitCount, srcBit, destBit) \
    ...
    // Peak at first byte to fetch opcode
    uint8_t opcode;
    EVMU_API_VERIFY(evmuInstructionPeak(pInstruction, &opcode));

    // Retrieve detailed instructoin format information
    const EvmuInstructionFormat* pFmt;
    EVMU_API_VERIFY(evmuInstructionFormat(opcode, &pFmt));

    // Clear operand results
    memset(pOperands, 0, sizeof(EvmuDecodedOperands));

    // Fetch argument count
    const uint8_t argc = EVMU_ISA_ARGC(pFmt->argFormat);

    // Don't fucking bother if there aren't any operands to decode.
    EVMU_CONDITIONAL_RETURN(argc == 0);

    // Concatenate bytes into a uint32 in reverse order for more easy processing.
    uint32_t instrCode = 0;
    for(uint32_t byte = 0; byte < pFmt->byteCount; ++byte) {
        const uint32_t destByte = pFmt->byteCount - 1 - byte;
        instrCode |= pInstruction->bytes[byte] << (8 * destByte);
    }
     ...
     */
    if(EVMU_ISA_ARG_FORMAT_EXTRACT(pFmt->argFormat, EVMU_ISA_ARG_FORMAT_FIELD_ARG1) == EVMU_ISA_ARG_TYPE_BIT_3 &&
            EVMU_ISA_ARG_FORMAT_EXTRACT(pFmt->argFormat, EVMU_ISA_ARG_FORMAT_FIELD_ARG2) == EVMU_ISA_ARG_TYPE_DIRECT_9)
    {
        if(EVMU_ISA_ARG_FORMAT_EXTRACT(pFmt->argFormat, EVMU_ISA_ARG_FORMAT_FIELD_ARG3) == EVMU_ISA_ARG_TYPE_RELATIVE_8) {
            EXTRACT_OPERAND(relative8.s8, 8u);
        }
    ...
    // Handle all other instruction types without dicks up their ass consistently.
        ...
        for(int a = (int)argc-1; a >= 0; --a) {
            const EVMU_ISA_ARG_TYPE argType = EVMU_ISA_ARG_FORMAT_EXTRACT(pFmt->argFormat, (unsigned)a);

            switch(argType) {
            case EVMU_ISA_ARG_TYPE_RELATIVE_8:
                EXTRACT_OPERAND(relative8.s8, 8);
                break;
            ...
            case EVMU_ISA_ARG_TYPE_ABSOLUTE_12: //So fucking stupid... Not contiguous.
                ...
                EXTRACT_OPERAND_APPEND(absolute, 1u, 1u, 11u);
                break;
